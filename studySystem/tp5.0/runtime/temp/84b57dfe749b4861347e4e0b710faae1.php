<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:98:"E:\phpStudy\PHPTutorial\WWW\tp5.0\public/../application/studysystem\view\index\editUserAcademy.php";i:1521545902;}*/ ?>
﻿<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <script>
        function save() {
            ...
            if(realname.length>0){
                var pattern1 =  /^[\u4E00-\u9FA5]+$/;
                verfiyName=pattern1.test(realname);
                if(verfiyName){
                    if(realname.length<=6){
                        if(number.length>0){
                            var pattern2 =  /^[0-9]+$/;
                            verfiyNumber=pattern2.test(number);
                            if(verfiyNumber){
                                if(number.length<=10){
        ...
        <div id="edit" align="center"
            ...
            <form name="update" action="/studySystem/updateUserAcademy" method="post">
                <label>
                    ...
                    <br>
                    <br>
                    学号 : <input id="number" name="number" style="width:120pt;color: black" value="<?php echo $number; ?>">
                    <br>
                    <br>
                    学院 :
